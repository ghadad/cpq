  ...
  <div class="flex-1 navbar">
      ...
      <li tabindex="0">
        ...
        <ul class="p-2 bg-base-100 z-50	">
             <ul class="ml-7">
                <li v-for="s in qm.sections"  @click="qm.setSection(s)"><router-link :to="'/qmanager/sections/'+s">{{s}}</router-link></li>
                </ul>
        </ul>
      </li>
      <li tabindex="0">
        ...
        <ul class="p-2 bg-base-100 z-50	">
          <li v-for="db in $qm.databases" @click="$qm.setDB(db)"> <a>{{ db }} </a></li>
           </ul>
      </li>
      <li><a> <ShareIcon class="h-5 w-5 text-blue-500"/></a></li>
    </ul>
    </div>
        <div class="flex-none"> 
    <label class="btn btn-round btn-ghost">
